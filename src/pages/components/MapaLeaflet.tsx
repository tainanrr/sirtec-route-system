...
export default function MapaLeaflet({ rotas, osPendentes, equipesMock, todasEquipes, equipeHovered, equipeEditando, osSelecionada, osSelecionadaNoEditor, focarOSNoMapa, onOSSelecionada, onIncluirOSNaRota, territorios = [], onTerritorioEditado, osUrgenteDestaque, osUrgentesDestaque, onOsUrgenteDestaqueClear, selecionandoCoordNoMapa, onMapClick, osCoordenadasSuspeitas = [], criandoPoligono, onPoligonoCriado, onCriacaoCancelada, onOsSelecionadasPorPoligono, ossSelecionadas, statusOSsTempoReal, mostrarEquipesTempoReal = true, equipesSelecionadasFiltro, onEquipeClick, prazoLimiteUrgente, versaoPrazoUrgente, previsoesChuvaPorData }: MapaLeafletProps) {
  ...
  useEffect(() => {
    ...
    // Log para debug
    console.log("[MapaLeaflet] Equipes com turno aberto:", equipesComTurno.length, equipesComTurno.map(e => ({ id: e.equipe_id, codigo: e.equipe_codigo, lat: e.ultima_latitude, lng: e.ultima_longitude })));
    console.log("[MapaLeaflet] Filtro de equipes selecionadas:", equipesSelecionadasFiltro?.length, equipesSelecionadasFiltro?.slice(0, 5));
    ...
    console.log("[MapaLeaflet] Equipes filtradas para exibição:", equipesFiltradas.length, equipesFiltradas.map(e => e.equipe_codigo));
  ...
  useEffect(() => {
    ...
    try {
      ...
      const criarPopupHTML = (os: OrdemServico) => {
        ...
        // Usar município e bairro da OS como localização
        const localPrevisao = [os.municipio, os.bairro].filter(Boolean).join(' - ') || 'Localização da OS';
        ...
        const previsaoTempoHTML = previsaoPopup ? `
          <div style="margin-top: 10px; padding: 8px; background: linear-gradient(135deg, #e0f2fe 0%, #bae6fd 100%); border-radius: 8px; border: 1px solid #7dd3fc;">
            ...
            <div style="display: flex; gap: 12px; font-size: 11px; color: #0369a1; flex-wrap: wrap;">
            ...
            <div style="margin-top: 6px; font-size: 10px; color: #0284c7; display: flex; align-items: center; gap: 4px;">
